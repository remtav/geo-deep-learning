...
from utils.utils import load_from_checkpoint, get_device_ids, gpu_stats, get_key_def, minmax_scale, list_input_images, \
    read_csv, add_metadata_from_raster_to_sample
            ...
            for row in tqdm(range(overlay, h + chunk_size, chunk_size - overlay), position=1, leave=False,
                ...
                with tqdm(range(overlay, w + chunk_size, chunk_size - overlay), position=2, leave=False, desc='Inferring columns') as _tqdm:
                    for col in _tqdm:
                        ...
                        totensor_transform = augmentation.compose_transforms(params, dataset="tst", type='totensor', random_radiom_trim_range=trim)
...
def main(params):
    ...
    out_file = working_folder / Path('inf_trim_metrics.csv')
        ...
        with tqdm(list_img, desc='image list', position=0) as _tqdm:
            for info in _tqdm:
                ...
                for i in list_trim_vals:
                    ...
                    vis(params, inf_sample['sat_img'], sem_seg_results_per_class, working_folder,
                        inference_input_path=local_img,
                        debug=debug,
                        trim=str(trim_at_inference).replace(".", ""))
